    ...
    for( auto &resource_dir : resource_dirs ) {
        read_from_file( resource_dir + "/" + FILENAMES[filename_label], [&]( std::istream &fin ) {
            ...
            if ( view_name.empty() || resource_name.empty() ) {
                DebugLog( D_ERROR, DC_ALL ) << "NAME and/or VIEW line missing from " << FILENAMES[filename_label] << " in " << resource_dir;
            } else if( resource_option.count( resource_name ) != 0 ) {
                DebugLog( D_ERROR, DC_ALL ) << "Found " << operation_name << " duplicate with name " << resource_name << " in " << FILENAMES[filename_label] << " in " << resource_dir;
            } else {
